 ...
 */
#ifndef PS_FMQ_VAN_H_
#define PS_FMQ_VAN_H_
#include <zmq.h>
#include <stdlib.h>
#include <thread>
#include <string>
 ...
 */
inline void FFreeData(void *data, void *hint) {
  if (hint == NULL) {
 ...
 protected:
  ...
  int Bind(const Node& node, int max_retry) override {
    ...
    //  if (zmq_bind(receiver_, address.c_str()) == 0) break;
      if(receiver_bind((Receiver *)receiver_, (int)port) == 0)break;
      if (i == max_retry) {
        port = -1;
      } else {
  ...
  void Connect(const Node& node) override {
    ...
    void *sender = socket_(RDMA_PS_TCP);
    ((Socket *)sender)->node_id = my_node_.id;
    CHECK(sender != NULL)
        << zmq_strerror(errno)
...
//    if (zmq_connect(sender, addr.c_str()) != 0) {
    char c_port[10];
    std::sprintf(c_port,"%d",node.port);
    if (!connect_((Socket **)&sender, node.hostname.c_str(), c_port, my_node_.id)) {
      LOG(FATAL) <<  "connect to " + node.hostname + " failed: " + zmq_strerror(errno);
    }
    senders_[id] = sender;
  }
  ...
  int SendMsg(const Message& msg) override {
    ...
    int tag = SND_MORE_FLAG;
    int n = msg.data.size();
    if (n == 0) tag = 0;
    AMessage *meta_msg = AMessage_create(meta_buf, meta_size, tag);
    //zmq_msg_init_data(&meta_msg, meta_buf, meta_size, FreeData, NULL);
    while (true) {
      //if (zmq_msg_send(&meta_msg, socket, tag) == meta_size) break;
      if(send_((Socket *)socket, meta_msg) == 0){
    	break;
      }
    ...
    FFreeData(meta_buf, NULL);
    AMessage_destroy((AMessage *)meta_msg);
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (i == n - 1) tag = 0;
      AMessage *data_msg = AMessage_create(data->data(), data->size(), tag);
      while (true) {
        // if (zmq_msg_send(&data_msg, socket, tag) == data_size) break;
	if(send_((Socket *)socket, data_msg) == 0){
	  break;
	}
        if (errno == EINTR) continue;
        LOG(WARNING) << "failed to send message to node [" << id
      ...
      FFreeData(data->data(),data);
      AMessage_destroy(data_msg);
  ...
  int RecvMsg(Message* msg) override {
    ...
    for (int i = 0; ; ++i) {
      ...
      //while (true) {
        ...
        zmsg = receiver_recv((Receiver *)receiver_);
//	printf("recv a message: length %d, node_id %d, flag: %d\n", zmsg->length, zmsg->node_id, zmsg->flag);
      ...
      //  return -1;
      //}
      char* buf = CHECK_NOTNULL((char *)(zmsg->buffer));
      size_t size = zmsg->length;
      recv_bytes += size;
	...
	msg->meta.sender = zmsg->node_id;
	msg->meta.recver = my_node_.id;
      //} else if (i == 1) {
        // task
        UnpackMeta(buf, size, &(msg->meta));
        bool more = (zmsg->flag == SND_MORE_FLAG);
        AMessage_destroy(zmsg);
        if (!more) break;
      } else {
        // zero-copy
        SArray<char> data;
        ...
        if (zmsg->flag != SND_MORE_FLAG) { break; }
      }
    }
    return recv_bytes;
  }
